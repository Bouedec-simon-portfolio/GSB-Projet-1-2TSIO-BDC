...
<?php if (session('user')): ?><nav><a href="<?= site_url('fiches') ?>">Mes fiches</a><a href="<?= site_url('administration') ?>">Administration</a></nav><form method="post" action="<?= site_url('deconnexion') ?>"><?= csrf_field() ?><button>Déconnexion</button></form><?php endif ?></header>
